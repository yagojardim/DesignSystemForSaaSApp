...
// Card registry — each report card
...
const REPORT_CARDS: ReportCardDef[] = [
  { id: 'burndown',    title: 'Burndown Chart',          subtitle: 'Sprint S14 · Story points restantes vs. ideal',           span2: true  },
  { id: 'velocity',    title: 'Velocity Chart',           subtitle: 'Story points entregues por sprint',                       span2: false },
  { id: 'cfd',         title: 'CFD / Cumulative Flow',    subtitle: 'Distribuição de itens por status ao longo do tempo',      span2: true  },
  { id: 'bugs',        title: 'Bugs por Severidade',      subtitle: 'Issues de tipo Bug abertas no sprint',                    span2: false },
  { id: 'criados',     title: 'Criados vs Resolvidos',    subtitle: 'Issues criadas e resolvidas por semana (8 semanas)',      span2: true  },
  { id: 'workload',    title: 'Workload por Pessoa',       subtitle: 'Story points atribuídos por membro da equipe',           span2: false },
  { id: 'aging',       title: 'Aging de Issues',          subtitle: 'Dias em aberto por issue em progresso',                  span2: false },
  { id: 'leadtime',    title: 'Lead Time & Cycle Time',   subtitle: 'Tempo médio de entrega e execução',                      span2: false },
  { id: 'health',      title: 'Saúde do Projeto',         subtitle: 'Score geral baseado em 5 dimensões',                     span2: false },
  { id: 'epic',        title: 'Epic / Release Burndown',  subtitle: 'Story points restantes por épico ao longo de 6 semanas', span2: true  },
]
...
// ── SVG Charts (unchanged internal implementations) ───────────────────────────
...
function BurndownChart() {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: 'visible' }}>
      ...
      {ticks.map(t => <text key={t} x={PAD.left - 6} y={toY(t) + 4} textAnchor="end" fontSize={9} fill={T.text3}>{t}</text>)}
      {dayTicks.map(d => <text key={d} x={toX(d)} y={H - PAD.bottom + 14} textAnchor="middle" fontSize={9} fill={T.text3}>D{d + 1}</text>)}
...
function VelocityChart() {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`}>
      ...
      {[0, 10, 20, 30].map(t => <text key={t} x={PAD.left - 4} y={toY(t) + 3} textAnchor="end" fontSize={8} fill={T.text3}>{t}</text>)}
    </svg>
  )
}
...
function CFDChart() {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: 'visible' }}>
      ...
      {[1, 4, 7, 10, 13].map(d => <text key={d} x={toX(d)} y={H - PAD.bottom + 14} textAnchor="middle" fontSize={9} fill={T.text3}>D{d + 1}</text>)}
      {[0, 10, 20, 30].map(t => <text key={t} x={PAD.left - 4} y={toY(t) + 4} textAnchor="end" fontSize={9} fill={T.text3}>{t}</text>)}
...
function CreatedVsResolved() {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: 'visible' }}>
      ...
      {Array.from({ length: weeks }, (_, i) => <text key={i} x={toX(i)} y={H - PAD.bottom + 14} textAnchor="middle" fontSize={9} fill={T.text3}>W{i + 1}</text>)}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: px(12) }}>
      ...
      <svg width="100%" viewBox={`0 0 ${W} ${H}`}>
        ...
        {[0, 2, 4, 6].map(t => <text key={t} x={PAD.left - 3} y={toY(t) + 3} textAnchor="end" fontSize={7} fill={T.text3}>{t}</text>)}
      </svg>
    </div>
  )
}
  ...
  return (
    <div style={{ display: 'flex', gap: px(16), alignItems: 'flex-start' }}>
      <svg width={170} height={150} viewBox="0 0 170 150">
        ...
        {axes.map((a, i) => { const [x, y] = point(i, 1.22); return <text key={i} x={x} y={y} textAnchor="middle" fontSize={8} fill={T.text2}>{a.label}</text> })}
...
function EpicBurndown() {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: 'visible' }}>
      ...
      {epics.map((e, i) => <path key={i} d={linePath(e.data)} stroke={e.color} strokeWidth={2} fill="none" />)}
      {epics.map((e, i) => e.data.map((v, j) => <circle key={`${i}-${j}`} cx={toX(j)} cy={toY(v)} r={3} fill={e.color} />))}
      {Array.from({ length: weeks }, (_, i) => <text key={i} x={toX(i)} y={H - PAD.bottom + 14} textAnchor="middle" fontSize={9} fill={T.text3}>W{i + 1}</text>)}
      {[0, 5, 10, 15, 20].map(t => <text key={t} x={PAD.left - 4} y={toY(t) + 4} textAnchor="end" fontSize={9} fill={T.text3}>{t}</text>)}
...
function CardContent({ id }: { id: string }) {
  switch (id) {
    ...
    case 'cfd':      return <CFDChart />
    case 'bugs':     return <BugsDonut />
    case 'criados':  return <CreatedVsResolved />
    case 'workload': return <WorkloadChart />
    case 'aging':    return <AgingChart />
    case 'leadtime': return <LeadCycleChart />
    case 'health':   return <ProjectHealth />
    case 'epic':     return <EpicBurndown />
    default:         return null
  }
}
